...
int IOInfoList::compareItems(IOInfo *ci1, IOInfo *ci2) {
  ...
  */
  ...
  // qDebug("IO %s %d  %s %d", io1->convertToString().latin1(), io1->getRole(),
  // io2->convertToString().latin1(), io2->getRole());
  if (s1 > s2)
    return 1;
  else if (s1 < s2)
    return -1;
    ...
    // qDebug("identical IO %s %s", io1->convertToString().latin1(),
    // io2->convertToString().latin1());
    /*
    if (io1->isInverted() && !io2->isInverted())
      return 1;
    else if (!io1->isInverted() && io2->isInverted())
...
bool IOInfoList::lessThan(IOInfo *&ci1, IOInfo *&ci2) {
  return (IOInfoList::compareItems(ci1, ci2) == -1);
}
