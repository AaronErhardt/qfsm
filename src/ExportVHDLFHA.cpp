...
ExportVHDLFHA::ExportVHDLFHA(Options *opt) : Export(opt) {
...
QString ExportVHDLFHA::fileFilter() { return "VHDL (*.vhdl *.vhd)"; }
  ...
  *out << "SIGNAL next_state, current_state : state_type;" << endl << endl;
  ...
  /*
    *out << "-- comments for syf which comes with alliance :
    http://www-asim.lip6.fr/alliance"<<endl; *out << "-- interpreted as comments
    by other tools"<<endl; *out << "  --pragma CURRENT_STATE current_state" <<
    endl; *out << "  --pragma NEXT_STATE next_state" << endl;
...
void ExportVHDLFHA::writeClockProcess() {
  ...
  if (synchronous_reset)
    ...
    *out << "  state_register: PROCESS (rst_n, clk)" << endl;
  ...
  *out << "    ELSIF rising_edge(clk) THEN" << endl;
  if (synchronous_reset) {
    ...
    *out << "        current_state <= "
         << Utils::noWS(istate->getStateName()).latin1() << ";" << endl;
...
void ExportVHDLFHA::writeStateProcess() {
  ...
  *out << ")\n";
  if (machine->getNumInputs() > 0)
    *out << "    VARIABLE temp_input : std_ulogic_vector("
         << machine->getNumInputs() - 1 << " DOWNTO 0);" << endl;
  if (machine->getNumMooreOutputs() > 0)
    *out << "    VARIABLE temp_output : std_ulogic_vector("
         << machine->getNumMooreOutputs() - 1 << " DOWNTO 0);" << endl;
  if (machine->getNumOutputs() > 0)
    *out << "    VARIABLE temp_mealy_output : std_ulogic_vector("
  ...
  *out << "    CASE current_state IS";
  ...
  for (; is.hasNext();) {
    ...
    if (s->countTransitions() > 0) {
      *out << endl
           << "      WHEN " << Utils::noWS(s->getStateName()).latin1() << " =>";
      ...
      *out << endl
           << "          WHEN OTHERS => next_state <= current_state;" << endl;
      *out << "        END CASE;" << endl;
    }
  }
...
void ExportVHDLFHA::writeHeader(QString commentstart, QString commentend) {
  ...
  *out << commentstart.latin1()
       << " Module     : " << machine->getName().latin1() << commentend.latin1()
       << endl;
  *out << commentstart.latin1()
       << "----------------------------------------------------------------"
       << commentend.latin1() << endl;
  *out << commentstart.latin1() << " Author     : " << commentend.latin1()
       << endl;
  *out << commentstart.latin1()
  ...
  *out << commentstart.latin1()
       << "----------------------------------------------------------------"
       << commentend.latin1() << endl;
  *out << commentstart.latin1() << " Description: " << commentend.latin1()
       << endl;
  *out << commentstart.latin1()
       << "----------------------------------------------------------------"
       << commentend.latin1() << endl;
  *out << commentstart.latin1()
       << " Revisions : " << machine->getVersion().latin1() << " - "
       << commentend.latin1() << endl;
  *out << commentstart.latin1()
       << "----------------------------------------------------------------"
       << commentend.latin1() << endl;
