...
void SimulatorDlgImpl::init() {
  ...
  if (isIBinChecked())
    simulatorDlg.le_inputs->setMaxLength(
...
void SimulatorDlgImpl::iBinPressed() {
  ...
  if (isIHexChecked()) {
    ...
    conv.resolveEscapes(simulatorDlg.le_inputs->text(), ascii,
...
void SimulatorDlgImpl::iASCIIPressed() {
  ...
  if (isIBinChecked()) {
    ...
    conv.binStrToASCII(simulatorDlg.le_inputs->text(), ascii,
                       MAX_CHARARRAY_LENGTH, alen);
    ...
    conv.hexStrToASCII(simulatorDlg.le_inputs->text(), ascii,
                       MAX_CHARARRAY_LENGTH, alen);
    if (alen > 0) {
      single = ascii[alen - 1];
...
void SimulatorDlgImpl::oBinPressed() {
  ...
  if (isOHexChecked()) {
    tmp = conv.hexStrToBinStr(main->project->machine->getNumOutputs(),
                              simulatorDlg.lb_outputs->text());
    simulatorDlg.lb_outputs->setText(tmp);
  } else if (isOASCIIChecked()) {
    unsigned char ascii[MAX_CHARARRAY_LENGTH];
    int alen;
    conv.resolveEscapes(simulatorDlg.lb_outputs->text(), ascii,
                        MAX_CHARARRAY_LENGTH, alen);
...
void SimulatorDlgImpl::oASCIIPressed() {
  ...
  if (isOBinChecked()) {
    ...
    conv.binStrToASCII(simulatorDlg.lb_outputs->text(), ascii,
    ...
    conv.hexStrToASCII(simulatorDlg.lb_outputs->text(), ascii,
                       MAX_CHARARRAY_LENGTH, alen, FALSE);
    tmp = conv.asciiToReadableStr(ascii, alen);
    simulatorDlg.lb_outputs->setText(tmp);
  }
