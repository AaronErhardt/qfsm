...
//  conv.invertX10(len, bininv, bin);
    ...
    if (simulatorDlg.pb_bin0->isEnabled())
        simulatorDlg.pb_bin0->setChecked(bin[0]=='1');
    if (simulatorDlg.pb_bin1->isEnabled())
        simulatorDlg.pb_bin1->setChecked(bin[1]=='1');
    if (simulatorDlg.pb_bin2->isEnabled())
        simulatorDlg.pb_bin2->setChecked(bin[2]=='1');
    if (simulatorDlg.pb_bin3->isEnabled())
        simulatorDlg.pb_bin3->setChecked(bin[3]=='1');
    if (simulatorDlg.pb_bin4->isEnabled())
        simulatorDlg.pb_bin4->setChecked(bin[4]=='1');
    if (simulatorDlg.pb_bin5->isEnabled())
        simulatorDlg.pb_bin5->setChecked(bin[5]=='1');
    if (simulatorDlg.pb_bin6->isEnabled())
        simulatorDlg.pb_bin6->setChecked(bin[6]=='1');
    if (simulatorDlg.pb_bin7->isEnabled())
        simulatorDlg.pb_bin7->setChecked(bin[7]=='1');
    if (simulatorDlg.pb_bin8->isEnabled())
        simulatorDlg.pb_bin8->setChecked(bin[8]=='1');
    if (simulatorDlg.pb_bin9->isEnabled())
        simulatorDlg.pb_bin9->setChecked(bin[9]=='1');
    if (simulatorDlg.pb_bin10->isEnabled())
        simulatorDlg.pb_bin10->setChecked(bin[10]=='1');
    if (simulatorDlg.pb_bin11->isEnabled())
        simulatorDlg.pb_bin11->setChecked(bin[11]=='1');
    if (simulatorDlg.pb_bin12->isEnabled())
        simulatorDlg.pb_bin12->setChecked(bin[12]=='1');
    if (simulatorDlg.pb_bin13->isEnabled())
        simulatorDlg.pb_bin13->setChecked(bin[13]=='1');
    if (simulatorDlg.pb_bin14->isEnabled())
        simulatorDlg.pb_bin14->setChecked(bin[14]=='1');
    if (simulatorDlg.pb_bin15->isEnabled())
        simulatorDlg.pb_bin15->setChecked(bin[15]=='1');
...
void SimulatorDlgImpl::updateInputsStr() {
    ...
    if (len>0)
        bin[0]=simulatorDlg.pb_bin0-> isChecked()+'0';
    if (len>1)
        bin[1]=simulatorDlg.pb_bin1-> isChecked()+'0';
    if (len>2)
        bin[2]=simulatorDlg.pb_bin2-> isChecked()+'0';
    if (len>3)
        bin[3]=simulatorDlg.pb_bin3-> isChecked()+'0';
    if (len>4)
        bin[4]=simulatorDlg.pb_bin4-> isChecked()+'0';
    if (len>5)
        bin[5]=simulatorDlg.pb_bin5-> isChecked()+'0';
    if (len>6)
        bin[6]=simulatorDlg.pb_bin6-> isChecked()+'0';
    if (len>7)
        bin[7]=simulatorDlg.pb_bin7-> isChecked()+'0';
    if (len>8)
        bin[8]=simulatorDlg.pb_bin8-> isChecked()+'0';
    if (len>9)
        bin[9]=simulatorDlg.pb_bin9-> isChecked()+'0';
    if (len>10)
        bin[10]=simulatorDlg.pb_bin10-> isChecked()+'0';
    if (len>11)
        bin[11]=simulatorDlg.pb_bin11-> isChecked()+'0';
    if (len>12)
        bin[12]=simulatorDlg.pb_bin12-> isChecked()+'0';
    if (len>13)
        bin[13]=simulatorDlg.pb_bin13-> isChecked()+'0';
    if (len>14)
        bin[14]=simulatorDlg.pb_bin14-> isChecked()+'0';
    if (len>15)
        bin[15]=simulatorDlg.pb_bin15-> isChecked()+'0';
...
void SimulatorDlgImpl::resetButtons() {
    ...
    resetBits();
    simulatorDlg.pb_clock->setChecked(false);
}


void SimulatorDlgImpl::resetBits() {
    simulatorDlg.pb_bin0->setChecked(false);
    simulatorDlg.pb_bin1->setChecked(false);
    simulatorDlg.pb_bin2->setChecked(false);
    simulatorDlg.pb_bin3->setChecked(false);
    simulatorDlg.pb_bin4->setChecked(false);
    simulatorDlg.pb_bin5->setChecked(false);
    simulatorDlg.pb_bin6->setChecked(false);
    simulatorDlg.pb_bin7->setChecked(false);
    simulatorDlg.pb_bin8->setChecked(false);
    simulatorDlg.pb_bin9->setChecked(false);
    simulatorDlg.pb_bin10->setChecked(false);
    simulatorDlg.pb_bin11->setChecked(false);
    simulatorDlg.pb_bin12->setChecked(false);
    simulatorDlg.pb_bin13->setChecked(false);
    simulatorDlg.pb_bin14->setChecked(false);
    simulatorDlg.pb_bin15->setChecked(false);
}
...
void SimulatorDlgImpl::setState(int s) {
    switch (s) {
    case Busy:
        simulatorDlg.pm_end->setPixmap(*redlight);
        break;
    case Finished:
        simulatorDlg.pm_end->setPixmap(*greenlight);
        break;
    default:
        simulatorDlg.pm_end->setPixmap(*greylight);
        break;
    }
}
